...
class Database:
    ...
    def create_vlan(self, vlan: Vlan, view_name: str):
        return self.run_sql_stmt("INSERT into vlan VALUES (%s,%s,%s)",(vlan.name, vlan.number, view_name), False)

    def get_vlan(self, vlan: Vlan, view_name: str):
        data = self.run_sql_stmt("SELECT * from vlan WHERE vlan_number=%s AND view_name=%s",(vlan.number, view_name), True)
        return Vlan(name=data[0], number=data[1])

    def delete_vlan(self, vlan: Vlan, view_name: str):
        return self.run_sql_stmt("DELETE FROM vlan WHERE vlan_number=%s AND view_name=%s", (vlan.number, view_name), False)
    ...
    def get_address(self, address: str, view_name: str):
        return self.run_sql_stmt("SELECT address from address WHERE address=%s and view_name=%s",
                          (address, view_name), True)

    def create_address(self, address: str, version: int, network: Network, view_name: str):
        return self.run_sql_stmt("""
            INSERT into address (address, version, network_id ,view_name) VALUES (%s,%s,(SELECT network_id from network WHERE network=%s and view_name=%s),%s) 
